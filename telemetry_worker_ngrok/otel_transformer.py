...
class OTELTransformer:
    ...
    def _convert_span(self, gcp_span: trace_v1.TraceSpan, metadata: Dict[str, Any]) -> Span:
        ...
        # Span ID (GCP uses decimal, OTEL expects hex bytes)
        if gcp_span.span_id:
            try:
                span_id_int = int(gcp_span.span_id)
                # Convert to 16-character hex string (8 bytes)
        ...
        # Parent Span ID (GCP uses decimal, OTEL expects hex bytes)
        if gcp_span.parent_span_id:
            try:
                parent_id_int = int(gcp_span.parent_span_id)
                # Convert to 16-character hex string (8 bytes)
                parent_id_hex = format(parent_id_int, '016x')
                span.parent_span_id = bytes.fromhex(parent_id_hex)
            except Exception as e:
